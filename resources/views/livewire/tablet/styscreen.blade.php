<div class="h-full flex flex-col space-y-6" wire:poll.10s>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-stone-200 pb-5">
        ...
        <div class="flex items-center gap-3">
            ...
            <!-- Logout button -->
            <x-ui.button variant="outline" size="sm" wire:click="logout" class="h-[38px] rounded-xl text-stone-600 hover:text-red-600 border-stone-200">
                    ...
                    <div class="border border-stone-150 rounded-2xl p-4 bg-stone-50/50 hover:bg-white hover:shadow-sm transition space-y-3">
                        <div class="flex justify-between items-start">
                            <div>
                                ...
                                <span class="text-stone-400 text-[10px] uppercase font-extrabold tracking-wide">{{ $b->booking_date->format('d M Y') }} @ {{ substr($firstItem->start_time, 0, 5) }}</span>
                            </div>
                        ...
                        <div class="text-xs space-y-1 text-stone-700">
                            ...
                            <p><strong>Stylist:</strong> {{ $b->stylist->name }}</p>
                            <p class="text-sm font-bold text-[#0A3D91] mt-2">Total: Rp {{ number_format($b->net_amount, 0, ',', '.') }}</p>
                        </div>

                        <div class="pt-2">
                            <x-ui.button variant="primary" size="sm" class="w-full rounded-xl text-xxs font-black uppercase tracking-wider h-[38px]" wire:click="selectBookingForPayment({{ $b->id }})">
                                Bayar via EDC / Tunai
                            </x-ui.button>
                        </div>
                    </div>
                ...
                @forelse($paidActiveBookings as $b)
                    ...
                    <div class="border border-stone-150 rounded-2xl p-4 bg-stone-50/50 hover:bg-white hover:shadow-sm transition space-y-3">
                        ...
                        <div class="pt-2 flex gap-2">
                            <x-ui.button variant="outline" size="sm" class="flex-1 rounded-xl text-xxs font-black uppercase tracking-wider h-[38px] border-stone-200" wire:click="selectBookingForPayment({{ $b->id }})">
                                Cetak Invoice
                            </x-ui.button>
                        </div>
                    </div>
            ...
            <div class="space-y-4 overflow-y-auto max-h-[600px] pr-1 flex-grow">
                @forelse($completedBookings as $b)
                    ...
                    <div class="border border-stone-150 rounded-2xl p-4 bg-emerald-50/10 hover:bg-white hover:shadow-sm transition space-y-3">
                        ...
                        <div class="pt-2">
                            <x-ui.button variant="outline" size="sm" class="w-full rounded-xl text-xxs font-black uppercase tracking-wider h-[38px] border-stone-200 hover:bg-stone-50" wire:click="selectBookingForPayment({{ $b->id }})">
                                Cetak Invoice / Resi
                            </x-ui.button>
                        </div>
                    </div>
    ...
    @if($selectedBooking)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-sm print:hidden">
            <div class="relative w-full max-w-lg bg-white rounded-3xl border border-stone-200 shadow-2xl p-8 space-y-6">
                <!-- Modal Close -->
                <button type="button" class="absolute top-6 right-6 text-stone-400 hover:text-stone-700" wire:click="closePaymentModal">
                        ...
                        <form wire:submit.prevent="processPayment" class="space-y-4">
                            <div>
                                <x-ui.select label="Metode Pembayaran" wire:model="paymentMethod">
                                    ...
                                    <x-ui.input label="Metode Lainnya" placeholder="e.g. Transfer" wire:model="paymentMethod" />
                                </x-ui.select>
                            </div>

                            <div>
                                <x-ui.input label="No. Referensi / EDC (Opsional)" placeholder="e.g. Ref: 827182" wire:model="transactionReference" />
                            </div>

                            <div class="pt-2 flex gap-3">
                                <x-ui.button variant="outline" type="button" class="flex-1 rounded-xl h-[46px]" wire:click="closePaymentModal">Batal</x-ui.button>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="border border-stone-200 rounded-2xl p-6 bg-stone-50 font-mono text-xs space-y-4 max-w-sm mx-auto shadow-inner bg-white" id="invoice-receipt">
                            ...
                            <div class="space-y-1 text-[10px] text-stone-700">
                                ...
                                <p><strong>Tgl:</strong> {{ $selectedBooking->booking_date->format('d/m/Y') }} @ {{ substr($selectedBooking->items->first()->start_time, 0, 5) }}</p>
                                <p><strong>Kasir:</strong> {{ auth()->user()->name }}</p>
                                <p><strong>Client:</strong> {{ $selectedBooking->customer->name }}</p>
                                <p><strong>Stylist:</strong> {{ $selectedBooking->stylist->name }}</p>
                                <div class="border-b border-dashed border-stone-300 my-2"></div>
                            </div>
                        ...
                        <div class="flex gap-3 pt-2">
                            <x-ui.button variant="outline" type="button" class="flex-1 rounded-xl h-[46px]" wire:click="closePaymentModal">Tutup</x-ui.button>
