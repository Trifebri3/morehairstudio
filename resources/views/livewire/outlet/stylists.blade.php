                    ...
                    <div class="border border-stone-200 rounded-2xl p-5 bg-white shadow-sm flex flex-col justify-between space-y-4 hover:border-[#0A3D91] transition duration-200">
                        ...
                        <div class="pt-4 border-t border-stone-100 flex justify-between items-center">
                            @if(in_array($stylist->status, ['pending_active', 'pending_inactive', 'pending_leave']))
                                <div class="flex flex-col space-y-2 w-full">
                                    ...
                                    <div class="flex space-x-2 pt-1">
                                        <x-ui.button class="flex-1 text-center justify-center bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] py-1" size="sm" wire:click="approveStatus({{ $stylist->id }})">
                                            Setuju
                                        </x-ui.button>
                                        <x-ui.button class="flex-1 text-center justify-center bg-rose-600 hover:bg-rose-700 text-white text-[10px] py-1" size="sm" wire:click="rejectStatus({{ $stylist->id }})">
                                            Tolak
                                        </x-ui.button>
                                    </div>
                                </div>
                            @else
                                ...
                                <x-ui.button variant="outline" size="sm" wire:click="toggleStatus({{ $stylist->id }})">
                                    Toggle Status
                                </x-ui.button>
                            @endif
                        </div>
