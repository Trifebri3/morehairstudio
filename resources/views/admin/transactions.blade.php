        ...
        <form method="GET" action="{{ route('admin.transactions') }}" id="transactions-filter-form" class="bg-white border border-stone-200 p-6 rounded-3xl shadow-sm space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-5 gap-3">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="search" value="{{ $search }}" placeholder="e.g. TX-2026... atau Budi" onchange="this.form.submit()" class="w-full text-xs rounded-xl border-stone-200 bg-stone-50/50 h-9 px-3 text-stone-750 placeholder-stone-400 focus:border-[#0A3D91] transition" />
                </div>

                <!-- Outlet Scope -->
                <div>
                    <label class="block text-[10px] uppercase font-bold text-stone-400 mb-1">Scope Outlet</label>
                    <select name="filterOutlet" onchange="this.form.submit()" class="w-full text-xs rounded-xl border-stone-200 bg-stone-50/50 h-9 px-3 text-stone-700" {{ auth()->user()->role === 'outlet_admin' ? 'disabled' : '' }}>
                        <option value="">Semua Outlet</option>
                        @foreach($outlets as $o)
                            <option value="{{ $o->id }}" {{ $filterOutlet == $o->id ? 'selected' : '' }}>{{ $o->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="filterPaymentMethod" onchange="this.form.submit()" class="w-full text-xs rounded-xl border-stone-200 bg-stone-50/50 h-9 px-3 text-stone-700">
                        <option value="">Semua Metode</option>
                        <option value="cash" {{ $filterPaymentMethod === 'cash' ? 'selected' : '' }}>Cash</option>
                        <option value="qris" {{ $filterPaymentMethod === 'qris' ? 'selected' : '' }}>QRIS</option>
                        <option value="transfer" {{ $filterPaymentMethod === 'transfer' ? 'selected' : '' }}>Bank Transfer</option>
                        <option value="ewallet" {{ $filterPaymentMethod === 'ewallet' ? 'selected' : '' }}>E-Wallet</option>
                    </select>
                </div>
                ...
                <div class="grid grid-cols-2 gap-2 sm:col-span-1">
                    <div>
                        ...
                        <input type="date" name="dateFrom" value="{{ $dateFrom }}" onchange="this.form.submit()" class="w-full text-xs rounded-xl border-stone-200 bg-stone-50/50 h-9 px-2 text-stone-770" />
                    </div>
                    <div>
                        <label class="block text-[9px] uppercase font-bold text-stone-400 mb-1">Sampai</label>
                        <input type="date" name="dateTo" value="{{ $dateTo }}" onchange="this.form.submit()" class="w-full text-xs rounded-xl border-stone-200 bg-stone-50/50 h-9 px-2 text-stone-770" />
                    </div>
                </div>
            </div>
        </form>
                        ...
                        @forelse($transactions as $tx)
                            <tr class="hover:bg-stone-50/50 transition">
                                ...
                                <td class="py-3 px-4 text-right space-x-2">
                                    <a href="?{{ http_build_query(array_merge(request()->query(), ['view_tx' => $tx->id])) }}" class="inline-flex items-center gap-1 px-3 py-1.5 border border-stone-200 bg-white text-stone-700 hover:bg-stone-50 rounded-lg text-xs font-bold transition">
                                    ...
                                    @if($tx->status === 'completed' && auth()->user()->role === 'super_admin')
                                        <form method="POST" action="{{ route('admin.transactions.refund', $tx->id) }}" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan & me-refund transaksi POS ini? Stok produk dan poin pelanggan akan ditarik kembali.')" class="inline">
                                            @csrf
                                            <button type="submit" class="h-7 px-3 text-[10px] font-bold uppercase tracking-wider bg-red-50 text-red-750 hover:bg-red-100 transition rounded-lg">
                                                Refund
                                            </button>
                                        </form>
                                    @endif
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto bg-stone-950/40 backdrop-blur-xs flex items-center justify-center p-4">
            <div class="bg-white border rounded-3xl p-6 max-w-sm w-full shadow-2xl space-y-6">
                ...
                <div class="flex justify-end gap-3 pt-2">
                    <a href="?{{ http_build_query(request()->except('view_tx')) }}" class="px-4 py-2 bg-stone-100 hover:bg-stone-200 text-stone-750 text-xxs font-bold uppercase tracking-wider rounded-xl transition border border-stone-200">Tutup</a>
