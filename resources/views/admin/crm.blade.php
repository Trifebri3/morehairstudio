    ...
    <form method="GET" action="{{ route('admin.crm') }}" id="crm-filter-form" class="bg-white border border-stone-200 p-6 rounded-3xl shadow-sm mb-8 space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-5 gap-3">
            ...
            <div>
                ...
                <select name="filterOutlet" onchange="this.form.submit()" class="w-full text-xs rounded-xl border-stone-200 bg-stone-50/50 h-9 px-3 text-stone-770" {{ auth()->user()->role === 'outlet_admin' ? 'disabled' : '' }}>
            ...
            <div>
                ...
                <select name="filterSource" onchange="this.form.submit()" class="w-full text-xs rounded-xl border-stone-200 bg-stone-50/50 h-9 px-3 text-stone-770">
            ...
            <div>
                ...
                <select name="filterSegment" onchange="this.form.submit()" class="w-full text-xs rounded-xl border-stone-200 bg-stone-50/50 h-9 px-3 text-stone-770">
                    ...
                    @foreach(['Champions', 'Loyal Customers', 'Potential Loyalists', 'New Customers', 'Promising', 'At Risk', 'Need Attention', 'Lost Customers'] as $seg)
                        <option value="{{ $seg }}" {{ $filterSegment === $seg ? 'selected' : '' }}>{{ $seg }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="date" name="dateFrom" value="{{ $dateFrom }}" onchange="this.form.submit()" class="w-full text-xs rounded-xl border-stone-200 bg-stone-50/50 h-9 px-3 text-stone-770" />
            </div>

            <!-- Date To -->
            <div>
                <label class="block text-[10px] uppercase font-bold text-stone-400 mb-1">Daftar Sampai</label>
                <input type="date" name="dateTo" value="{{ $dateTo }}" onchange="this.form.submit()" class="w-full text-xs rounded-xl border-stone-200 bg-stone-50/50 h-9 px-3 text-stone-770" />
            </div>
        </div>
    </form>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1 space-y-4">
            <div class="bg-white border border-stone-200 p-6 rounded-3xl shadow-sm">
                <div class="mb-4">
                    <input type="text" name="search" form="crm-filter-form" value="{{ $search }}" placeholder="Cari Nama / Kode / Telepon..." class="w-full text-xs rounded-xl border-stone-200 bg-stone-50/50 h-10 px-4 text-stone-750 placeholder-stone-400 focus:border-[#0A3D91] transition" />
                </div>
                                ...
                                <div class="relative">
                                    ...
                                    <div class="bg-stone-50/50 border p-4 rounded-2xl space-y-2 text-xxs">
                                        <div class="flex justify-between items-center">
                                            <span class="font-black uppercase tracking-wider text-stone-750">{{ str_replace('_', ' ', $act->event_type) }}</span>
                                        ...
                                        @if($act->metadata)
                                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-2 text-stone-500 font-mono">
                                                @foreach((array)$act->metadata as $mKey => $mVal)
                                                    @if(is_array($mVal))
