            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-1 space-y-8">
                    ...
                    <div class="glass-panel p-6 rounded-2xl bg-white border border-stone-200 shadow-sm space-y-4">
                        ...
                        <div class="pt-2">
                            @if ($stylist->status === 'active')
                                <button wire:click="requestLeave" class="w-full text-center justify-center inline-flex items-center px-4 py-2.5 bg-rose-50 border border-rose-200 rounded-xl font-bold text-xs text-rose-700 hover:bg-rose-100 transition shadow-sm">
                                    Ajukan Cuti / Nonaktif
                                </button>
                            @elseif ($stylist->status === 'inactive')
                                <button wire:click="requestActivate" class="w-full text-center justify-center inline-flex items-center px-4 py-2.5 bg-emerald-50 border border-emerald-200 rounded-xl font-bold text-xs text-emerald-700 hover:bg-emerald-100 transition shadow-sm">
                    ...
                    <div class="glass-panel p-6 rounded-2xl bg-white border border-stone-200 shadow-sm space-y-6">
                        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center border-b border-stone-100 pb-4 gap-4">
                            ...
                            <div class="flex items-center space-x-2 w-full sm:max-w-xs">
                                ...
                                <input type="date" wire:model.live="searchDate" class="w-full text-xs border-stone-300 focus:border-brand-500 focus:ring-brand-500 rounded-xl shadow-sm px-3.5 py-2">
                            </div>
                        </div>
                        ...
                        <div class="bg-stone-50/50 p-4 rounded-2xl border border-stone-100">
                            <div class="flex items-center justify-between overflow-x-auto space-x-3 pb-2 scrollbar-none scroll-smooth">
                                @foreach($weekDays as $day)
                                    <button 
                                        wire:click="$set('searchDate', '{{ $day['date'] }}')" 
                                    ...
                                    <div class="flex items-center justify-between md:justify-end space-x-4 pt-2 md:pt-0 border-t md:border-t-0 border-stone-200/80">
                                        <div class="flex items-center space-x-2 w-full justify-between md:justify-end">
                                            ...
                                            @if ($booking->status === 'pending')
                                                <button wire:click="confirmBooking({{ $booking->id }})" class="px-4 py-2 bg-brand-50 hover:bg-brand-100 text-brand-600 font-extrabold text-[10px] uppercase rounded-xl tracking-wider transition border border-brand-100 shadow-sm">
                                                    Konfirmasi
                                                </button>
                                            @elseif ($booking->status === 'confirmed')
                                                <button wire:click="completeBooking({{ $booking->id }})" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-[10px] uppercase rounded-xl tracking-wider transition shadow-sm">
                                                    Selesaikan
                                                </button>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="glass-panel p-6 rounded-2xl bg-white border border-stone-200 shadow-sm space-y-4">
                            ...
                            <form wire:submit="updateProfile" class="space-y-4">
                                <div>
                                    <x-input-label for="editName" value="Nama Lengkap" class="text-stone-700 font-semibold mb-1 text-[11px]" />
                                    <x-text-input wire:model="editName" id="editName" class="block w-full px-3 py-2 text-xs rounded-xl border border-stone-300 focus:border-brand-500 focus:ring-brand-500 shadow-sm bg-white" required />
                                    <x-input-error :messages="$errors->get('editName')" class="mt-1" />
                                </div>

                                <div>
                                    <x-input-label for="editEmail" value="Alamat Email (Asli)" class="text-stone-700 font-semibold mb-1 text-[11px]" />
                                    <x-text-input wire:model="editEmail" id="editEmail" class="block w-full px-3 py-2 text-xs rounded-xl border border-stone-300 focus:border-brand-500 focus:ring-brand-500 shadow-sm bg-white" required type="email" />
                                    <x-input-error :messages="$errors->get('editEmail')" class="mt-1" />
                                </div>

                                <div>
                                    <x-input-label for="editPhone" value="No. WhatsApp (Awali 62)" class="text-stone-700 font-semibold mb-1 text-[11px]" />
                                    <x-text-input wire:model="editPhone" id="editPhone" class="block w-full px-3 py-2 text-xs rounded-xl border border-stone-300 focus:border-brand-500 focus:ring-brand-500 shadow-sm bg-white" required placeholder="628123456789" />
                                    <x-input-error :messages="$errors->get('editPhone')" class="mt-1" />
                                </div>

                                <div>
                                    <x-input-label for="editSpecialization" value="Spesialisasi" class="text-stone-700 font-semibold mb-1 text-[11px]" />
                                    <x-text-input wire:model="editSpecialization" id="editSpecialization" class="block w-full px-3 py-2 text-xs rounded-xl border border-stone-300 focus:border-brand-500 focus:ring-brand-500 shadow-sm bg-white" required placeholder="Contoh: Hair Coloring, Fade Expert" />
                                    <x-input-error :messages="$errors->get('editSpecialization')" class="mt-1" />
                                </div>

                                <div>
                                    <x-input-label for="editBio" value="Biografi Singkat" class="text-stone-700 font-semibold mb-1 text-[11px]" />
                                    <textarea wire:model="editBio" id="editBio" rows="3" class="block w-full text-xs border-stone-300 focus:border-brand-500 focus:ring-brand-500 rounded-xl shadow-sm px-3 py-2 bg-white"></textarea>
