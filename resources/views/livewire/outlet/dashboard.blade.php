<div>
    ...
    <form wire:submit="saveSettings" class="space-y-8 mb-10">
        ...
        <div class="glass-panel p-8 rounded-3xl bg-white border border-stone-200 shadow-sm space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div>
                    <x-ui.input label="Absen Masuk Mulai" type="time" wire:model="attendanceStart" />
                </div>
                <div>
                    <x-ui.input label="Absen Masuk Batas Akhir" type="time" wire:model="attendanceEnd" />
                </div>
                <div>
                    <x-ui.input label="Absen Pulang Mulai" type="time" wire:model="clockOutStart" />
                </div>
                <div>
                    <x-ui.input label="Absen Pulang Batas Akhir" type="time" wire:model="clockOutEnd" />
                </div>
            </div>
        </div>
        ...
        <div class="glass-panel p-8 rounded-3xl bg-white border border-stone-200 shadow-sm space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                <div>
                    <x-ui.input label="Batas Minimum Booking Online (Jam)" type="number" min="0" max="72" wire:model="bookingLeadTime" />
                </div>
                <div>
                    <x-ui.select label="Auto-Cancel Grace Period" wire:model.live="checkinGraceActive">
                        <option value="1">Aktif (Auto-Cancel)</option>
                        <option value="0">Nonaktif</option>
                    </x-ui.select>
                </div>
                <div>
                    <x-ui.input label="Batas Waktu Hadir (Menit)" type="number" min="1" max="180" wire:model="checkinGraceMinutes" :disabled="!$checkinGraceActive" />
                </div>
            </div>
        </div>
        ...
        <div class="glass-panel p-8 rounded-3xl bg-white border border-stone-200 shadow-sm space-y-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    ></textarea>
                    @error('description') <span class="text-red-500 text-xxs block mt-1">{{ $message }}</span> @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        ></textarea>
                        @error('mapIframe') <span class="text-red-500 text-xxs block mt-1">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        ...
                        <div class="space-y-3">
                            ...
                            @if(count($gallery) > 0)
                                <div class="grid grid-cols-3 sm:grid-cols-4 gap-2 border p-3.5 rounded-xl bg-stone-50/50 max-h-40 overflow-y-auto">
                                    @foreach($gallery as $idx => $photoUrl)
                                        <div class="relative group aspect-square rounded-lg overflow-hidden border bg-white">
                                            <img src="{{ $photoUrl }}" class="w-full h-full object-cover" />
                                            <button type="button" wire:click="removePhoto({{ $idx }})" class="absolute inset-0 bg-black/45 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition text-[10px] font-bold uppercase tracking-wider">
                                                Hapus
                                            </button>
                                        </div>
                                    @endforeach
                            ...
                            <div class="relative border-2 border-dashed border-stone-250 hover:border-stone-400 p-4 rounded-xl text-center bg-stone-50/10 cursor-pointer transition">
                                ...
                                <div class="space-y-1.5 pointer-events-none">
                                    <svg class="w-6 h-6 mx-auto text-stone-450" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <!-- Temp upload status -->
                            <div wire:loading wire:target="newPhotos" class="text-xxs text-[#0A3D91] font-bold">
                                Sedang mengunggah file sementara...
                            </div>

                            @error('newPhotos.*') <span class="text-red-500 text-xxs block mt-1">{{ $message }}</span> @enderror
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        @foreach($services as $s)
                            <tr class="hover:bg-stone-50/50 transition">
                                <td class="p-4 text-center">
                                    <input 
                                        type="checkbox" 
                                        wire:model="selectedServices.{{ $s->id }}" 
                                ...
                                <td class="p-4 font-mono text-stone-500">Rp {{ number_format($s->price, 0, ',', '.') }}</td>
                                <td class="p-4">
                                    <input 
                                        type="number" 
                                        wire:model="customPrices.{{ $s->id }}" 
                                        placeholder="{{ (int)$s->price }}"
                                        class="w-32 px-3 py-1.5 border border-stone-200 rounded-lg text-xs font-mono focus:ring-[#0A3D91] focus:border-[#0A3D91] outline-none"
                                        :disabled="!$selectedServices[{{ $s->id }}]"
                                    />
                                </td>
                                <td class="p-4 text-stone-500 font-sans">{{ $s->duration }} Menit</td>
                                <td class="p-4 font-sans">
                                    <input 
                                        type="number" 
                                        wire:model="customDurations.{{ $s->id }}" 
                                        placeholder="{{ $s->duration }}"
                                        class="w-24 px-3 py-1.5 border border-stone-200 rounded-lg text-xs focus:ring-[#0A3D91] focus:border-[#0A3D91] outline-none"
                                        :disabled="!$selectedServices[{{ $s->id }}]"
                                    />
                                </td>
                            </tr>
